...
async function handleViewPagination(hasNextPageFromServer: boolean): Promise<void> {
  ...
  const response = await sendRecord({ type: "view_lessons", page: currentPage }); // Request only the current page
  ...
  if (!response.lessons || response.lessons.length === 0) {
    ...
    response.lessons.forEach((lesson, index) => {
      console.log(
        `${index + 1}. ID: ${lesson.id} - ${lesson.title.trim()} - ${lesson.desc.trim()}`,
      );
    });
  }
  ...
  if (choice === "n" && hasNextPage) {
    currentPage++;
    await handleViewPagination(hasNextPage);
...
async function showMenu(): Promise<void> {
  ...
  console.log("5. Exit");
    ...
    //   await handleViewPagination(response.hasNextPage ?? false);
    //   return;
    // } else {
    //   console.log("\nNo lessons available to show.");
    // }
    currentPage = 0;
    await handleViewPagination(true);
    return;
  } else if (choice === "3") {
